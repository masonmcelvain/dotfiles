...
- **Local review loop**: review a Claude diff like a GitHub PR without leaving the terminal - `hxr` (bash) opens the files this branch has modified vs the default branch (diffed from the merge base, so upstream drift is ignored) plus untracked files, `M r` (helix) drops a line-numbered `REVIEW:<start>[-<end>]` comment below the selected line(s), and the `/resolve` command (`dot_claude/commands/resolve.md`) has Claude collect the markers, fix or push back on each, and delete them
